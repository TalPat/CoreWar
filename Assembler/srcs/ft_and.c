...
/*   Updated: 2018/09/14 13:16:37 by tpatter          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_andparam1(t_cw *cw, t_pc *pc, int *val, int *newidx)
{
	int	*arr;
...
void	ft_andparam2(t_cw *cw, t_pc *pc, int *val, int *newidx)
{
	int	*arr;
...
void	ft_and(t_cw *cw, t_pc *pc)
{
	...
	if (pc->cr == pc->cyccomplete)
	{
		...
		{
			ft_andparam1(cw, pc, val, &newidx);
			ft_andparam2(cw, pc, val, &newidx);
			val[2] = val[1] & val[0];
			if (!val[2])
				pc->carry = 1;
